...
<Steps>
  <Step title="Create a verification">
    ...
    <Accordion title="Skip phone verification with `phone`" icon="phone">
      ...
      If you've already verified the user's phone number on your end (e.g. your own SMS login, an authenticated session), pass it in `phone`. The returned `verification_url` will land the user directly on the liveness check — no phone-OTP step. Same field, same code path on your side; only the URL shape changes.
      ...
      ```json Response
      {
        "verification_url": "https://verifyyou.com/auto-login?phoneNumber=%2B12105550142&phoneCode=550142&session_id=zsjmq6",
        "external_id": "d4f8e2a1-..."
      }
      ```
