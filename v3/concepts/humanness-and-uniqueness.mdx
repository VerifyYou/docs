...
A confirmation reports `verified: true` only when the person cleared **both** checks. The confirmation also carries a `status` (`approved`, `flagged`, or `denied`) and the `reasons` behind it, such as `collision`. You confirm that result on your backend; see [Confirmations](/v3/dev/spec/server-api/confirmations).
