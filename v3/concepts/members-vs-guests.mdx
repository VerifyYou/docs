...
- **How it's collected.** `anonymous` skips identity entirely (guests only). `collection` has the person enter their own email or phone in the flow. `provided` means your server supplies it at [initialize](/v3/dev/spec/server-api/initialize), and the person can't swap it for a different one.
- **Whether you receive it.** Identity **sharing** controls whether the verified email or phone is returned to you on the [confirmation](/v3/dev/spec/server-api/confirmations). Off by default: we prove ownership without exposing the value.
...
<Note>
  ...
  [initialize](/v3/dev/spec/server-api/initialize). With nothing bound and identity optional,
