...
- Up to 50 keys per session. Keys up to 64 characters, values up to 1024, and a value can be a string, a number or a boolean.
...
<Update label="September 9, 2026" description="Skip window">
...
<Update label="September 8, 2026" description="Auto-return">

## Return the moment a check settles

[`POST /v3/initialize`](/v3/dev/spec/server-api/initialize) now takes `auto_return`. Set it and an approved check sends the person straight back to `redirect_url` the instant it settles, with no result screen and nothing to tap. Leave it off and they see the result screen and tap to return, as before.
...
<Update label="September 2, 2026" description="Usage caps">

## A new denial reason: `usage_cap_exceeded`

Every account has a default daily limit on verifications (66k). A run that lands after the day's limit has been reached now comes back denied with [`reasons: ["usage_cap_exceeded"]`](/v3/dev/spec/server-api/confirmations) instead of reaching the verification step.

- The limit resets daily at 00:00 UTC. Don't retry a capped run in a loop; it fails the same way until then.
- If you need your daily verification limit adjusted due to a launch, campaign or general traffic, [contact us](mailto:support@verifyyou.com) so we can adjust your limits.

Nothing to upgrade. This works with the SDK version you already run.

</Update>

<Update label="August 31, 2026" description="Groups">

## One group can now see another

[Groups](/v3/dev/integrating/start-a-session#groups) scope uniqueness to a pool, and until now every pool was sealed off from the others. [`POST /v3/initialize`](/v3/dev/spec/server-api/initialize) now takes `include_groups`: other groups whose history also counts against the run. To keep everyone who already passed `spring-intake` out of a follow-up, mint the follow-up's sessions with their own `group` plus `include_groups: ["spring-intake"]`.
...
- The confirmation now carries [`groups`](/v3/dev/spec/server-api/confirmations), the pools the `reasons` came from, on flagged passes too. It reads `[]` when there is nothing to report.
