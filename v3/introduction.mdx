...
<Steps>
  <Step title="Start a verification">
    From the browser with the SDK's [`vycheck()`](/v3/dev/spec/browser-sdk/vycheck), or from your server with [`POST /v3/initialize`](/v3/dev/spec/server-api/initialize). The flow can run as a full-page redirect, a drawer that slides over your page, or an inline embed. Pick the presentation in [`init()`](/v3/dev/spec/browser-sdk/init).
  </Step>
  ...
  <Step title="The result comes back to your app">
    Redirect mode returns the user to your page with `?vyt=<token>&vyc=<0|1>` on the URL. The embedded modes hand the same result to your `onComplete` callback with no redirect. Either way, read it with [`vyget()`](/v3/dev/spec/browser-sdk/vyget).
  </Step>
...
<CardGroup cols={2}>
  <Card title="init(): setup and display modes" icon="sliders" href="/v3/dev/spec/browser-sdk/init">
  ...
  <Card title="POST /v3/initialize" icon="server" href="/v3/dev/spec/server-api/initialize">
