...
<Tabs>
  ...
  <Tab title="Without a token">
    ...
    if (res.status === 404) {
      ...
      // your account's daily usage cap (no run is recorded to look up either way)
...
<Note>
  ...
  those checks the `vyt` is your only route to the verdict. See [Anonymous
  checks](/v3/dev/integrating/start-a-session#anonymous-checks).
</Note>
