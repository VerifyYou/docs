...
- A `404` from the token-free lookup isn't a broken session, just no confirmed pass yet — don't retry it in a loop. If the account is at its [daily usage cap](/v3/faq#what-happens-if-we-exceed-our-daily-limit), retries keep failing until it resets.
