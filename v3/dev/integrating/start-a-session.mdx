...
const res = await fetch("https://trust.verifyyou.com/v3/initialize", {
  ...
  body: JSON.stringify({
    ...
    // one uniqueness pool per group, so the same person can pass once in each
    group: "spring-intake",
...
Uniqueness covers the whole verification, so once someone passes, every later run
through that link reads as a duplicate. Reuse one link across separate
populations and that becomes the bug: whoever passed your spring intake is not a
duplicate when the autumn one opens, and blocking them is wrong. Send a `group`,
your own id for it, and the run is judged only against passes carrying the same
one, so each population gets its own "one person, once". Whatever string arrives
is the pool, nothing to register, and leaving it off judges the run against every
pass, as before.
