...
<AccordionGroup>
  ...
  <Accordion title="Can I trust the ?vyc value on the return URL?">
    ...
    [confirm it](/v3/dev/spec/server-api/confirmations) with your secret key. That result is
    authoritative.
  </Accordion>
  ...
  <Accordion title="Which key do I use where?">
    ...
    keys (`pk_…`) are deprecated. Never ship a secret key to the client.
  </Accordion>

  <Accordion title="How do test and live modes work?">
    Each key comes in a test and a live flavor (`sk_test_…` / `sk_live_…`).
  ...
  <Accordion title="Why did a returning person get blocked?">
    ...
    [verified phone or email](/v3/concepts/identity-and-recognition) so they
  ...
  <Accordion title="Does a person have to verify every time?">
    ...
    more often. See [Returning people](/v3/concepts/returning-users).
  </Accordion>
  ...
  <Accordion title="How do I tie a verification to my own account records?">
    ...
    [initialize](/v3/dev/spec/server-api/initialize) from your server.
    Duplicates are reported against your id. See
    [Account linking](/v3/advanced/account-linking).
  </Accordion>
  ...
  <Accordion title="What happens if we exceed our daily verification limit?">
    ...
    reached, runs come back denied with
    [`reasons: ["usage_cap_exceeded"]`](/v3/dev/spec/server-api/confirmations)
    for the rest of the day; it resets at 00:00 UTC. This is about your account,
    not the person being verified. Expecting high traffic due to a launch, a
    campaign, or general traffic? [Contact us](mailto:support@verifyyou.com)
    beforehand so we can size the limit with you.
  </Accordion>
