...
Most error responses have the shape `{ "detail": "<string>" }`. Exceptions:

- `422` from Pydantic body validation returns `{ "detail": [{ "type": "...", "loc": [...], "msg": "..." }] }`.
- `422` from `config_incomplete` returns `{ "detail": "<string>" }` — a plain string, not an array.
...
| `401` | `invalid_key_type` | Bearer token is not a `pk_*` or `sk_*` key. |
| `401` | `invalid_or_revoked_key` | Key not found or has been revoked. |
| `401` | `Not authenticated` | `Authorization` header missing entirely. |
| `403` | `config_requires_test_key` | `config` field sent with a live key — test keys only. |
| `404` | `no_matching_verification` | No verification matches the `origin` / `start_path` / `return_path` for your account. |
| `422` | _(array)_ | Request body failed Pydantic validation. |
| `422` | `config_incomplete: missing <fields>` | `config` was supplied but required fields are absent. |
| `502` | _(proxied)_ | VerifyYou's backend or AWS is temporarily unavailable. Retry with exponential back-off. |
